...
docker run -d --name bridgx_api --add-host host.docker.internal:host-gateway -v $(pwd)/conf/config.yml.mac:/home/tiger/api/conf/config.yml.prod -p 9090:9090 galaxyfuture/bridgx-api:v0.1.0 bin/wait-for-api.sh
# deploy sheduler
